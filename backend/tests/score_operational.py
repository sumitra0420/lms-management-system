...
converse() wrapper) — not an estimate. Architecture-agnostic: works on
either the consistency or verify architecture's output, since both save
the same tokens/elapsed_sec fields.

Run from inside backend/ (after run_integration_test.py has produced the
run):
    python tests/score_operational.py integration/result_claude_nova_260827
