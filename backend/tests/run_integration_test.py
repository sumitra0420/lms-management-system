...
# response — wrapping converse() directly means this works regardless of
# which model pair is active in .env, with no changes needed to
# extractor.py/consistency.py. Same technique used on the verify-flow
# branch's run_integration_test.py, ported here so both architectures'
# results are comparable on tokens/time, not just accuracy.
...
def _token_totals() -> dict:
    ...
    called right after check_with_retry() returns, before the next file's
...
def _append_token_csv_row(branch: str, filename: str, attempt: int, tokens: dict):
    ...
    row = {
        "branch":    branch,
        "arch":      "run_integration_test.py (consistency, main branch)",
        "filename":  filename,
